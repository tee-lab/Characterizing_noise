...
% stochastic process using the standard algorithm presented by Gillespie, 
% 1976. The process simulates the evolution of temporal dynamics of a 
% population that contains individuals in two states. These individuals 
% switch between their states via. different kinds of reactions/intercations. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


tic
mu  = 8; % Number of reactions at individual level
N = 200; % Population size
C = zeros(mu,1); %Array to store reaction rates propensities
A = C; %Array to store reaction propensities at the given time
r1 = 0.01; % spontaneous reaction rate
r2 = 1;    % pairwise reaction rate
r3 = 0.08; % Ternary reaction rate
r4 = 0;  % pairwise negative reaction rate
C(1:2) = r1;
C(3:4) = r2;
C(5:6) = r3;
C(7:8) = r4;

Tint = 1;%ceil((r2+r3+r4)/((N^0.5)*r1)); % Sampling time (Time after which system's state would be stored)
Tend = ceil(Tint*100000); % Simulation time. I have kept it as a function of sampling time. 
steps = floor(Tend); % Size of array to store system's state. Usually this will exceed the actual steps it Tint > 1
rel = 1;    %realizations/repetitions of the simulations
...
for iter = 1:rel
    iter;
    X1 = ceil(rand(1,1)*N); X2 = N-X1; %Initializing number of individuals in a given state
%     X1 = (randi(2)-1)*N; X2 = N-X1;
    n = 0; % counter to store system's state and time
    T = 0; % Intitializing system's time
%     loop = 0;
    Tprint = 0.01; %Initializing time at which sampling is done. This will get incremented by Tint after every sampling
    while (T < Tend)
        %         loop = loop + 1
        ...
        T = T + (log(1/R(1)))/A0;   %Calculating time to next reaction according to total reaction propensity
...
% Calculating coorelation time
    ...
    est_tau = tSample(ct);
    % Expected correlation time according to the deterministic term of the
    % SDE
